...
def check_drinks(drink_name, drink_ingredients):
    ...
    """
    if len(set(drink_ingredients).intersection(ALCOHOLS)):
...
def singleton_ingredients(dishes, intersection):
    ...
    # Create a dictionary to count the occurrences of each ingredient
    ingredient_counts = {}

    # Iterate through each dish's ingredient set and update ingredient counts
    for dish in dishes:
        for ingredient in dish:
            ingredient_counts[ingredient] = ingredient_counts.get(ingredient, 0) + 1

    # Collect the ingredients that appear only once in the dishes
    singleton_ingredients_set = {
        ingredient for ingredient, count in ingredient_counts.items() if count == 1
    }
